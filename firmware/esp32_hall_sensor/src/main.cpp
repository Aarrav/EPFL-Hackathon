...
#ifndef HALL_SENSOR_ACTIVE_HIGH
#define HALL_SENSOR_ACTIVE_HIGH 0
#endif

#ifndef HALL_SENSOR_COOLDOWN_MS
#define HALL_SENSOR_COOLDOWN_MS 5000
#endif
...
// Device metadata
const char* deviceName = "LeRobot Harvester ESP32";
...
const unsigned long COOLDOWN_MS = HALL_SENSOR_COOLDOWN_MS;
...
String buildJson() {
  ...
  json += String(counter);
  json += ",\"plucked\":";
  json += (magnetDetected ? "true" : "false");
  json += ",\"cooldown\":";
  json += String(millis() - lastTriggerTime);
  json += ",\"cooldownMs\":";
  json += String(COOLDOWN_MS);
  json += ",\"uptimeMs\":";
  json += String(millis());
  json += ",\"wifiRssi\":";
...
void loop() {
  ...
  bool currentMagnet = HALL_SENSOR_ACTIVE_HIGH ? (sensorState == HIGH) : (sensorState == LOW);
  unsigned long now   = millis();

  // Detect a fresh transition into the configured active state.
  // Count only once per cooldown window.
