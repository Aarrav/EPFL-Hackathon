...
class FruitGripperGUI:
    def __init__(self, root):
        ...
        self.root.configure(bg="#0F0F0F") # Pitch Black for contrast

        try:
            self.ser = serial.Serial(port=SERIAL_PORT, baudrate=BAUD_RATE, timeout=0.05)
            self.connected = True
        except:
        ...
        self.current_target = GRIPPER_OPEN
    ...
    def setup_ui(self):
        ...
        self.load_display = tk.Label(self.load_frame, text="0", font=("Impact", 64), fg="#FFFFFF", bg="#0F0F0F")
        self.load_display.pack()
        tk.Label(self.load_frame, text="CURRENT LOAD UNITS", font=("Helvetica", 10, "bold"), fg="#AAAAAA", bg="#0F0F0F").pack()
        ...
        self.status_text = tk.Label(self.status_card, text="READY / IDLE", font=("Helvetica", 14, "bold"), fg="#444444", bg="#1E1E1E")
        ...
        # Threshold Slider
        tk.Label(ctrl_card, text="PRESSURE THRESHOLD (MAX 180)", fg="#00FFCC", bg="#1E1E1E", font=("Helvetica", 10, "bold")).pack(anchor="w")
        self.threshold_slider = tk.Scale(ctrl_card, from_=10, to=MAX_PRESSURE_CAP, orient="horizontal", 
                                         bg="#1E1E1E", fg="white", highlightthickness=0, troughcolor="#333333", 
                                         activebackground="#00FFCC", font=("Helvetica", 10))
        self.threshold_slider.set(80)
        self.threshold_slider.pack(fill="x", pady=(5, 25))

        # Speed Slider
        tk.Label(ctrl_card, text="GRIP SPEED", fg="#00FFCC", bg="#1E1E1E", font=("Helvetica", 10, "bold")).pack(anchor="w")
        self.speed_slider = tk.Scale(ctrl_card, from_=50, to=1000, orient="horizontal", 
                                     bg="#1E1E1E", fg="white", highlightthickness=0, troughcolor="#333333", 
                                     activebackground="#00FFCC", font=("Helvetica", 10))
        ...
        self.btn_open = tk.Button(btn_frame, text="RELEASE", command=self.open_gripper, 
                                  bg="#10B981", fg="black", font=("Helvetica", 12, "bold"), 
                                  relief="flat", height=2, cursor="hand2", activebackground="#059669")
        self.btn_open.pack(side="left", expand=True, fill="x", padx=5)

        # Grip Button - Electric Blue
        self.btn_close = tk.Button(btn_frame, text="GRIP FRUIT", command=self.start_close, 
                                   bg="#3B82F6", fg="black", font=("Helvetica", 12, "bold"), 
                                   relief="flat", height=2, cursor="hand2", activebackground="#2563EB")
        self.btn_close.pack(side="left", expand=True, fill="x", padx=5)

        # E-Stop - Vibrant Red
        self.btn_stop = tk.Button(self.root, text="STOP SYSTEM", command=self.stop_all, 
                                  bg="#EF4444", fg="black", font=("Helvetica", 11, "bold"), 
                                  relief="flat", pady=15, cursor="hand2", activebackground="#DC2626")
    ...
    def send_command(self, reg, params):
        ...
        length = len(params) + 3
        packet = [0xFF, 0xFF, SERVO_ID, length, 0x03, reg] + params
    ...
    def close_process(self):
        while self.is_running and self.current_target > GRIPPER_CLOSED_LIMIT:
            load = self.read_load()
        ...
        if self.is_holding:
            self.status_text.config(text="● FRUIT DETECTED - HOLDING", fg="#FFFFFF", bg="#10B981")
    ...
    def update_loop(self):
        ...
        if load > self.threshold_slider.get():
            self.load_display.config(fg="#10B981") # Success Green
        elif load > self.threshold_slider.get() * 0.7:
            self.load_display.config(fg="#F59E0B") # Warning Orange
        else:
